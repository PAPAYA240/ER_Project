...
//-------------------------------------------------------------------------------------
// Fog of War (FOW) Inputs (추가)
//-------------------------------------------------------------------------------------
TEXTURE2D(_VisionMapTex);        // 비전 맵 텍스처 변수 선언
SAMPLER(sampler_VisionMapTex);  // 비전 맵 샘플러 변수 선언

float4 _FogMapCenter; // 비전 맵 카메라의 월드 XZ 중심 위치 (C#에서 설정)
float _FogMapScale; // 비전 맵 카메라의 Orthographic Size (C#에서 설정)
float _FogDarkness; // 안개 영역의 어두운 정도 (C#에서 설정)
