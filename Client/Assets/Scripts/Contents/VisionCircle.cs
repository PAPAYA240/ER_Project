...
public class VisionCircle : MonoBehaviour
{
    ...
    [Range(3, 100)]
    public int segments = 54;           // 원 둘레를 구성하는 세그먼트(분할) 수 (최소 3개)
    ...
    void GenerateCircleMesh()
    {
        ...
        int[] triangles = new int[segments * 3]; // segments * 3개의 정점 인덱스
        for (int i = 0; i < segments; i++)
        {
            triangles[i * 3] = 0;             // 중심점 인덱스
            triangles[i * 3 + 1] = i + 1;     // 현재 둘레 정점 인덱스
            triangles[i * 3 + 2] = i + 2 > segments ? 1 : i + 2; // 다음 둘레 정점 인덱스 (마지막은 첫 번째와 연결)
        }
        ...
        if (meshRenderer.sharedMaterial == null)
        {
            ...
            //meshRenderer.sharedMaterial = new Material(Shader.Find("FogShader")); // URP에서는 "Universal Render Pipeline/Lit"
        }
    }

    public void SetActivate(bool isActivate)
    {
        meshRenderer.enabled = isActivate;
    }
}